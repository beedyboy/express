...
$(document).on('click', '.modProductImg', function(){
 var id = $(this).attr("id"); 
 
   $.ajax({
    url: uri+'Product/editImage/'+id, 
       success:function(data)
      { 
   $('.headerTitle').html("Modify Product Image");
   $('.popup-content').html(data);
           showMod();
...
$(document).on('submit','.updatePImage', function(evt)
{
